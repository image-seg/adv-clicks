...
class DeepLabV3Plus(nn.Module):
    def __init__(self, backbone='resnet50', norm_layer=nn.BatchNorm2d,
        ...
        self.backbone = ResNetBackbone(backbone=self.backbone_name, pretrained_base=False,
                                       norm_layer=self.backbone_norm_layer,weight_dir=weight_dir, **kwargs)

        self.head = _DeepLabHead(in_channels=ch + 32, mid_channels=ch, out_channels=48,
    ...
    def load_pretrained_weights(self):
        pretrained = ResNetBackbone(backbone=self.backbone_name, pretrained_base=True,
                                    norm_layer=self.backbone_norm_layer,weight_dir=self.weight_dir, **self._kwargs)
    ...
    def forward(self, x, additional_features):
        ...
        c1, _, _, c4 = self.backbone(x, additional_features)
        ...
        if torch.are_deterministic_algorithms_enabled():
            x = DetrministicBilinear2D(x, size=c1.size()[2:], align_corners=True)
        else:
...
class _AsppPooling(nn.Module):
    ...
    def forward(self, x):
        ...
        if torch.are_deterministic_algorithms_enabled():
            return DetrministicBilinear2D(pool, size=x.size()[2:], align_corners=True)
        else:
