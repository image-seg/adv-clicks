...
class HighResolutionNet(nn.Module):
    ...
    def aggregate_hrnet_features(self, x):
        ...
        if torch.are_deterministic_algorithms_enabled():
            x1 = DetrministicBilinear2D(x[1], size=(x0_h, x0_w), align_corners=self.align_corners)
            x2 = DetrministicBilinear2D(x[2], size=(x0_h, x0_w), align_corners=self.align_corners)
            x3 = DetrministicBilinear2D(x[3], size=(x0_h, x0_w), align_corners=self.align_corners)
