...
class GrabCutDataset(ISDataset):
    def __init__(self, dataset_path, args,
        ...
        if args.n_samples:
            self.dataset_samples = [x.name for x in sorted(self._images_path.glob('*.*'))][:args.n_samples]
        else:
    ...
    def get_sample(self, index) -> DSample:
        ...
        return DSample(image, instances_mask, objects_ids=[1], ignore_ids=[-1], sample_id=index, imname=image_path)
