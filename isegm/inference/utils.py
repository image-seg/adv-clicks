...
from isegm.inference.datasets import normalize_dataset_name
...
def setup_deterministic(seed, is_full=True):
    """
    Задать начальные значения генераторов случайных чисел.
    """
...
def get_results_table(noc_list, over_max_list, brs_type, dataset_name, mean_spc, elapsed_time,
    ...
    table_header = (f'|{"Режим BRS":^13}|{"Датасет":^11}|'
                    f'{"NoC@80%":^9}|{"NoC@85%":^9}|{"NoC@90%":^9}|'
                    f'{">="+str(n_clicks)+"@85%":^9}|{">="+str(n_clicks)+"@90%":^9}|'
                    f'{"с/клик":^7}|{"Время":^9}|')
    row_width = len(table_header)

    header = f'Результаты оценки модели: {model_name}\n' if model_name is not None else ''
    header += '-' * row_width + '\n'
    header += table_header + '\n' + '-' * row_width
